...
export const FabricFiltersDrawer = ({
  ...
  const initialFilterTypes = searchParams.get("filterTypes")?.split(",") || []
  ...
  const [selectedFilterTypes, setSelectedFilterTypes] = useState<string[]>(initialFilterTypes)
  ...
  const handleFilterTypeChange = (filterType: string, checked: boolean) => {
    setSelectedFilterTypes((prev) => {
      const newValue = checked ? [...prev, filterType] : prev.filter((t) => t !== filterType)
      setFiltersChanged(true)
      return newValue
    })
  }
  ...
  }, [selectedCollections, selectedTypes, selectedFilterTypes, minAbrasion, maxAbrasion, selectedAvailabilities, filtersChanged, pathname])
      ...
      <div ref={drawerRef} className={`${styles.drawer} ${isOpen ? styles.open : ""}`}>
        ...
        {filterTypes.length > 0 && (
            <div className={styles.filterGroup}>
              <h3 className={styles.filterGroupTitle}>Производитель</h3>
              <div className={styles.filterOptions}>
                {filterTypes.map((filterType) => (
                  <label key={filterType.id} className={styles.filterOption}>
                    <input
                      type="checkbox"
                      checked={selectedFilterTypes.includes(filterType.id)}
                      onChange={(e) => handleFilterTypeChange(filterType.id, e.target.checked)}
                      className={styles.filterInput}
                    />
                    <span className={styles.filterLabel}>{filterType.name}</span>
                  </label>
                ))}
              </div>
            </div>
